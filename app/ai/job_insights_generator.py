...
class OpenAIJobInsightsGenerator:
    ...
    def _json(self, *, action: str, resume, job, job_page_text) -> dict:
        messages = [{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": build_insights_prompt(action=action, resume=resume, job=job, job_page_text=job_page_text)}]
...
class GeminiJobInsightsGenerator(OpenAIJobInsightsGenerator):
    ...
    def _json(self, *, action: str, resume, job, job_page_text) -> dict:
        payload = {"model": self.model, "input": f"{SYSTEM_PROMPT}\n{build_insights_prompt(action=action, resume=resume, job=job, job_page_text=job_page_text)}"}
        import urllib.request
        req = urllib.request.Request("https://generativelanguage.googleapis.com/v1beta/interactions", data=json.dumps(payload).encode(), headers={"Content-Type": "application/json", "x-goog-api-key": self.api_key}, method="POST")
        with urllib.request.urlopen(req) as response:
            content = _extract_gemini_text(json.loads(response.read().decode()))
        try:
            return json.loads(content.strip().removeprefix("```json").removesuffix("```").strip())
